...
#include<string>
#include<fstream>
bankAccount** readFile(string filepath, int &n);
void printAccounts(bankAccount **arr1, int size) ;
bool makeTransaction(bankAccount**arr1, int size, int id, float amount );
////void deleteAccounts(....);
//
void main()

{
	...
	int noofacc;
	bankAccount **arr1 = readFile("data.txt", noofacc);
...
//	*/
	makeTransaction(arr1, noofacc, 5, 6000);
//
//	/********************************TASK 9*************************************
...
//
//
bankAccount **readFile(string filepath, int &noofacc )
{
	...
	infile>>noofacc;
	bankAccount **arr2=new bankAccount*[noofacc];
	for(int i=0; i<noofacc;i++)
	{
		...
		arr2[i] = new bankAccount(id, balance, transaction, nooftrans, balancelimit);
	}
	return arr2;
}
...
{
	int type = 0;
	cout << "to deposit, enter 1 and to withdraw, enter 2";
	cin >> type;
	for (int i = 0; i < size; i ++)
	{
		if (arr1[i]->getID()==id)
		{
			if (type == 1)
			{
				arr1[i]->deposit(amount);
				cout << "successful transaction";
			}
			else if (type == 2)
			{
				arr1[i]->withdraw(amount);
